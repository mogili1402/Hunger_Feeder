...
router.post("/auth/signup", middleware.ensureNotLoggedIn, async (req,res) => {
	...
	const { firstName, lastName, email, password1, password2, role ,phone,address} = req.body;
	let errors = [];
	...
	if (phone.length!=0 && isValidPhone(phone)===false){
		errors.push({ msg: "Enter a Valid phone number"})
	}
	if(errors.length > 0) {
		return res.render("auth/signup", {
			title: "User Signup",
			errors, firstName, lastName, email, password1, password2,phone,address
		});
	}
	...
	try
	{
		...
		const newUser = new User({ firstName, lastName, email, password:password1, role ,phone,address});
